...
if ! command -v docker >/dev/null 2>&1 || ! docker compose version >/dev/null 2>&1; then
  printf '%s\n' "Docker Compose is required for conversation integration validation." >&2
  exit 1
fi

export JANUARY_ENVIRONMENT=test
...
for _ in $(seq 1 30); do
  if docker compose exec -T database pg_isready -U "$JANUARY_DATABASE_USER" -d "$JANUARY_DATABASE_NAME" >/dev/null 2>&1 \
    && docker compose exec -T redis redis-cli ping >/dev/null 2>&1; then
    break
  fi
  sleep 1
done
if ! docker compose exec -T database pg_isready -U "$JANUARY_DATABASE_USER" -d "$JANUARY_DATABASE_NAME" >/dev/null \
  || ! docker compose exec -T redis redis-cli ping >/dev/null 2>&1; then
...
"$uv_bin" run pytest -m conversation_integration
